...
$sqlw = mysqli_query($con, "SELECT * FROM books WHERE id_kategori = '$kategori'") or die(mysqli_error($con));
  $sqlu = mysqli_query($con, "SELECT * FROM kategori WHERE id_kategori = '$kategori'") or die(mysqli_error($con));
$daq= mysqli_fetch_array($sqlu);
?>
<!DOCTYPE html>
<html>
<head>
...
?>
  ...
  <div class="categories-otD">
    <p class="kategori-jWy">Kategori <?=$daq['nama_kategori']?></p>
    <div class="container-4ZF">
      ...
      <?php while ($dato = mysqli_fetch_array($sqlw)) { ?>
        <div class="shot-YUP">
          ...
          <button type="submit" value="<?=$dato['id_kategori']?>" name="wis" class="badge-Ekt">Wishlist</button>
  ...
  $mapel = mysqli_query($con,"SELECT * FROM playlists WHERE id_user = '$id_login'");
          ...
          <button type="submit" value="<?=$dato['ISBN']?>" name="deta" class="badge-Ekr">Detail Buku</button>
        </div>
        <?php 
          }
          ?>
