    ...
    <div class="account-section-7Wh">
      ...
      <div class="detail-sEH">
        <form method="post" action="" class="form-RFo" enctype="multipart/form-data">
          <p class="detail-akun-M9T">Edit Password</p>
          <div class="name-673">
            <p class="nama-lengkap--qqK">Konfirmasi Password Lama</p>
            <input name="pwlama" type="password" class="input-xuw">
          </div>
          <div class="name-673">
            <p class="nama-lengkap--qqK">Password Baru</p>
            <input type="password" name="pw" class="input-xuw">
          </div>
          <div class="name-EsT">
            <p class="tampilan-nama--oQm">Konfirmasi Password Baru </p>
            <input type="password" name="konfirpw" class="input-Xbf">
          </div>
          <!-- <div class="name-JXs">
            <p class="asal-instansi-Tff">Foto</p>
            <input name="foto" type="file" class="input-BrZ" value="
          </div> -->
        ...
        $jml=0;
        if ($_POST['pwlama']==$pass) {
          if ($_POST['pw']==$_POST['konfirpw']){
            $jml=1;
            $edit1= mysqli_query($con,"UPDATE users SET password='$_POST[pw]' WHERE id_user='$id_login' ");
            echo "<script>alert('Password Berhasil dirubah');
				window.location.replace('halaman-akun.php');
    </script>";
        }
        else{
          echo "<script>alert('Password baru anda tidak sama dengan konfirmasi password baru anda!!!');
    </script>";
        }
      }
      else {
