...
<div class="cart-oaH">
      ...
      <div class="cart-bQu">
        ...
        <table  class="elements-cart-product-cell-3fX">
          ...
          <tr>
            ...
            <td><p class="tere-liye-bumi-Xj7"><?=$data['Penulis']?> - <?=$data['Judul']?> <br> <a class="get-started-rL9" href="#">X Remove</a></p></td>
          ...
          <table>
          <tr>
            <td colspan="3"><button type="submit" name="pinjam"  class="button-yoP">Pinjam Sekarang</button></td>
          </tr>
          <tr>
            <td colspan=""><button type="submit" name="reservasi" class="button-eed">Reservasi</button></td>
          </tr> 
        </table>
        </form>
        <?php 
  if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $tanggal_kembali = date('Y-m-d', strtotime($tanggal_hari_ini. ' + 7 days'));
    $sql_nambah = mysqli_query($con, "INSERT INTO peminjaman VALUES('','$tanggal_hari_ini','$tanggal_kembali','$id','$id_login')");
    $sql111 = mysqli_query($con , "SELECT * FROM peminjaman WHERE id_mitra = '$id' AND id_user = '$id_login'");
    $data10 = mysqli_fetch_array($sql111);
    while ($tes = mysqli_fetch_assoc($sql10)){
      $sql90 = mysqli_query($con, "INSERT INTO buku_pinjam VALUES('','$data10[id_peminjaman]','$tes[ISBN]','$tes[id_mitra]','$id_login')");
    }
    $sql_hapus1 = mysqli_query($con, "DELETE FROM cart WHERE id_mitra = '$id' AND id_user = '$id_login'");
        echo "<script>
