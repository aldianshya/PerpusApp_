...
$sql12 = mysqli_query($con, "SELECT * FROM reservasi 
    ...
    <div class="cart-3eH">
      <div class="cart-bQu">
        ...
        <table  class="elements-cart-product-cell-3fX">
          <tr>
            <th class="buku-FVT">Tanggal Reservasi</th>
            <th class="buku-FVT">Perpustakaan</th>
            <th class="perpustakaan-ywF">Status</th>
            <th class="perpustakaan-ywF"></th>
          </tr>
          <?php while ($data = mysqli_fetch_array($sql12)) { ?>
          <tr>
            <td><p class="tere-liye-bumi-Xj7"><?=$data['tanggal_peminjaman']?></p></td>
            <td><p class="tere-liye-bumi-Xj7"><?=$data['Penulis']?><?=$data['nama_mitra']?></p></td>
            <td><p class="perpustakaan-universitas-jambi-mCD">
             Masih Diajukan
              </p></td>
              <td><button style="background-image: black; background-color:black; color:white; text-align:center;" class="tere-liye-bumi-Xj7" type="submit" value="<?=$data['id_reservasi']?>" name="detail">Detail</button></td>
          </tr>
          <?php } ?>
          </table>
        </form>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    if (isset($_POST["detail"])) {
      ...
      $_SESSION['users'] = $d['id_user'];
        echo "<script>
      window.location.href ='halaman-deskripsi-reservasi.php';
      </script>";
      }
  }
  ?>
