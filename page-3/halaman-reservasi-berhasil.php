...
WHERE cart.id_user = '$id_login' AND cart.id_mitra = '$id'") or die(mysqli_error($con));
        $tanggal = date("Y-m-d");
$sqpo = mysqli_query($con, "SELECT * FROM buku_reservasi_unconfir 
...
WHERE buku_reservasi_unconfir.id_reservasi = '$id_r'");
$sqp = mysqli_query($con, "SELECT * FROM buku_reservasi_unconfir 
JOIN reservasi ON reservasi.id_reservasi = buku_reservasi_unconfir.id_reservasi
JOIN books ON books.ISBN = buku_reservasi_unconfir.ISBN
JOIN mitra ON mitra.id_mitra = buku_reservasi_unconfir.id_mitra
WHERE buku_reservasi_unconfir.id_reservasi = '$id_r'");
$total = mysqli_num_rows($sqp);
$dara = mysqli_fetch_array($sqp)
?>
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="complete-6Uy">
    ...
    <div class="order-complete-tfj">
      ...
      <div id="kartuContent" class="orders-1-MSq">
        <div class="item-tSm">
          <?php while ($data = mysqli_fetch_array($sqpo)) { ?>
          <div class="image-placeholder-2Yy">
            <img class="paste-image-yj7" style="justify-content: center;"src="./assets/buku/<?=$data['Cover']?>"/>
          </div>
          <?php } ?>
        </div>
        <div class="order-detail-XPF">
          <div class="title-3sP">
            ...
            <div class="perpustakaan--1Bw" style="margin-right:1.0rem;">Perpustakaan :</div>
            <div class="perpustakaan--1Bw" style="margin-top: 2.0rem; margin-right:4.5rem;">Catatan :</div>
          </div>
          <div class="info-LED">
            <div class="october-31-2023-tFj"><?=$dara['tanggal_peminjaman']?></div>
            <div class="buku-pfB"><?=$total?> Buku</div>
            <?php $sat = mysqli_fetch_array($sql2);
            ?>
            <div class="perpustakaan-universitas-jambi-Zcm"><?=$dara['nama_mitra']?></div>
            <div class="perpustakaan-universitas-jambi-Zcm"><?=$dara['catatan']?></div>
          </div>
        </div>
      </div>
    </div>
    <button name="kembali" class="button-qPF">Kembali</button>
    <?php 
